        ...
        <div className="relative w-full">
            ...
            <Carousel className="w-full px-12" setApi={setApi} opts={{
                ...
                <CarouselContent className="-ml-3">
                    ...
                    {!isLoading ? (
                        <>
                            <CarouselItem className="pl-3 basis-auto" onClick={() => onSelect && onSelect(null)}>
                                <Badge className="rounded-lg px-3 py-1 cursor-pointer whitespace-nowrap text-xs" variant={!value ? "default" : "secondary"}>All</Badge>
                            </CarouselItem>

                            {data?.map((item, index) => (
                                <CarouselItem className="pl-3 basis-auto" key={index} onClick={() => onSelect && onSelect(item.value)}>
